...
public class servicet implements Initializable {
    @FXML
    private TableView<t3> table;
    @FXML
    private TableColumn<t3, String> id3;
    @FXML
    private TableColumn<t3, String> fname3;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            pscheck = connectdb6.prepareStatement("select * from service ");
            resultSet = pscheck.executeQuery();
            while (resultSet.next()) {
                oblist.add(new t3(resultSet.getString("sid"),
    ...
    public void delbutton(ActionEvent actionEvent) {
        ...
        ResultSet resultSet = null;
        try {
            pscheck = connectdb7.prepareStatement("delete from service where sid ='" + deletetext.getText() + "'");
        ...
        try {
            pscheck = connectdb7.prepareStatement("select * from service ");
            resultSet = pscheck.executeQuery();
            while (resultSet.next()) {
                oblist.add(new t3(resultSet.getString("sid"),
                        resultSet.getString("sname")));
    ...
    public  void submit(ActionEvent actionEvent) throws FileNotFoundException {
        ...
        ResultSet resultSet = null;
        try {
            psinsert = connectdb4.prepareStatement(" Insert into service(slogo,sname) values (?,?)");
        ...
        oblist.removeAll(oblist);
        try {
            pscheck = connectdb4.prepareStatement("select * from service");
            resultSet = pscheck.executeQuery();
            while (resultSet.next()) {
                oblist.add(new t3(resultSet.getString("sid"),
                        resultSet.getString("sname")));
